...
struct eap_oob_serv_data{
        ...
        uint32 version[MAX_SUP_VER];
        char * ssid;
        char * peerID;
        uint32 state;
        uint32 cryptosuite[MAX_SUP_CSUITES];
        uint32 dir;
        uint32 minsleep;
...
int ECDH_KDF_X9_63(unsigned char * , size_t ,
...
static int eap_oob_get_hoob(struct eap_oob_peer_context *,unsigned char *, size_t );
